    ...
    const columns = [
...

        {
          ...
          render: (text, record, index) => (
            <Button  onClick={()=> 
            {setRedeemAmount(record.DepositAccountValue); setRedeemCoin(record.Coin);setRedeemInterest(record.InterestAmount); setModalOpen(true)}}>
              {"Redeem"}
            </Button>)
        },
      ...
      return (
      <>
      <History onClick={()=>navigate(`/termdepositwallethistory/${currentUser._id}`)}>History</History>
      ...
      <MyModal isOpen={modalOpen}
        ...
        <h1>
            ...
            <Highlight>{redeemAmount} {redeemCoin}</Highlight>
          </h1>
          <h1>
